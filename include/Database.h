...
#ifdef SQLITE_USE_LEGACY_STRUCT // Since SQLITE 3.19 (used by default since SQLiteCpp 2.1.0)
...
public:
     ...
     *
     */
    bool open(const int   aBusyTimeoutMs = 0);
     ...
     */
    long long getLastInsertRowid() const ; // nothrow

    /// Get total number of rows modified by all INSERT, UPDATE or DELETE statement since connection (not DROP table).
    int getTotalChanges() const ; // nothrow

    /// Return the numeric result code for the most recent failed API call (if any).
    int getErrorCode() const ; // nothrow
    /// Return the extended numeric result code for the most recent failed API call (if any).
    int getExtendedErrorCode() const ; // nothrow
    /// Return UTF-8 encoded English language explanation of the most recent failed API call (if any).
    const char* getErrorMsg() const ; // nothrow

    /// Return the filename used to open the database.
    const std::string& getFilename() const  // nothrow
    {
        return mFilename;
    }
     ...
     */
    inline sqlite3* getHandle() const  // nothrow
    {
        return mpSQLite;
    }

...
const char* getLibVersion()  // nothrow
{
    return sqlite3_libversion();
}

// Return SQLite version number using runtime call to the compiled library
int getLibVersionNumber()  // nothrow
{
    return sqlite3_libversion_number();
}

Database::Database(const char* apFilename)
	:mFilename(apFilename), mpSQLite(NULL)
{

}

// Open the provided database UTF-8 filename with SQLite::OPEN_xxx provided flags.
bool Database::open(const int   aFlags,
                    const int   aBusyTimeoutMs /* = 0 */,
                    const char* apVfs          /* = NULL*/)
{
	const int ret = sqlite3_open_v2(mFilename.c_str(), &mpSQLite, aFlags, apVfs);
    if (SQLITE_OK != ret)
    {    		
		return false;   
	}
...
// Open the provided database UTF-8 filename with SQLite::OPEN_xxx provided flags.
bool Database::open(const int   aBusyTimeoutMs)
{
    const int ret = sqlite3_open(mFilename.c_str(), &mpSQLite);
    if (SQLITE_OK != ret)
    {
       return false;   
    }
...
long long Database::getLastInsertRowid() const  // nothrow
{
...
int Database::getTotalChanges() const  // nothrow
{
    return sqlite3_total_changes(mpSQLite);
}

// Return the numeric result code for the most recent failed API call (if any).
int Database::getErrorCode() const  // nothrow
{
    return sqlite3_errcode(mpSQLite);
}

// Return the extended numeric result code for the most recent failed API call (if any).
int Database::getExtendedErrorCode() const  // nothrow
{
    return sqlite3_extended_errcode(mpSQLite);
}

// Return UTF-8 encoded English language explanation of the most recent failed API call (if any).
const char* Database::getErrorMsg() const  // nothrow
{
